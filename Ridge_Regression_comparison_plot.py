...
for horizon in range(15):
    ...
    HORIZON = horizon # <-- Set your desired horizon here (weeks ahead)
    ...
    if municipio_info["name"] not in adjacency_list:
        raise KeyError(f"{municipio_info['name']} not found in adjacency list; check name mapping / GeoJSON NOME field")
...
x_values = np.arange(1, len(y_values)+1)  # or your actual HORIZON values
plt.figure(figsize=(5, 3), dpi=300)  # smaller figure

plt.plot(x_values, y_values, marker='o', markersize=5, linestyle='-', color='tab:blue', label=r'$R^2$')
...
plt.tight_layout()
plt.savefig('horizon_r2_plot.png', dpi=300, bbox_inches='tight')
import csv
...
print(f"Saved R² values to {output_file}")



# -----------------------------
# SHAP explanations for the single horizon
# -----------------------------
